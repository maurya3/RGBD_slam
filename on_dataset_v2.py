...
from on_dataset import visualize_matches
...
# px_ref, desr = detector.detectAndCompute(imgr, mask=None)
...
for img_id in range(2000):
    ...
    px_ref, desr = detector.detectAndCompute(imgr, mask=None)
    px_cur, desc = detector.detectAndCompute(imgc, mask=None)

    matcher = cv2.BFMatcher_create(cv2.NORM_L2, crossCheck=False)
    matches = matcher.knnMatch(desr, desc, k=2)
    filtered_match = []
    for m, n in matches:
        if m.distance <= 0.5*n.distance:
            filtered_match.append(m)

    image1_points = np.float32([px_ref[m.queryIdx].pt for m in filtered_match])
    image2_points = np.float32([px_cur[m.trainIdx].pt for m in filtered_match])

    E = cv2.findEssentialMat(image1_points, image2_points, camMatrix)[0]
    _, R, t, mask = cv2.recoverPose(E, image1_points, image2_points, camMatrix)
    ...
    # px_ref = px_ref[st==1]
    # kp2 = kp2[st==1]
    out_image = visualize_matches(imgr,px_ref,imgc,px_cur)
    #out_img = cv2.drawMatches(imgc, image2_points,imgr, image1_points, matches1to2=filtered_match, outImg=None)
    #out_image = draw_match_2_side(imgr,px_ref,imgc,px_cur,len(image1_points))
    imS = cv2. resize(out_image, (1080, 320)) 
    cv2.imshow("out_image",imS)
    ...
    x = float(cur_t[0]) +180.10663266278726 # 180.10663266278735 -180.10663266278738
    y = float(cur_t[1]) -180.10663266278735
    z = float(cur_t[2]) +180.10663266278738
    
    # ##08
    # x = float(cur_t[0])# +180.10663266278726 # 180.10663266278735 -180.10663266278738
    # y = float(cur_t[1]) #-180.10663266278735
    # z = float(cur_t[2]) #-232.81490719936338 #476.9007448262875 -99.7929341559875 232.81490719936338
    # x = 0.95*xp + 0.05*x
    # y = 0.95*yp + 0.05*y
    # z = 0.95*xp + 0.05*z
